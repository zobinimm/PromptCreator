...
text = "赵默笙独自推着购物车，艰难的在人群中走走停停。刚刚从国外回来的她，还不太适应这样的拥挤，然而这样热闹而亲切的场面，却使她不自觉的带着微笑。她不知道别人刚刚回国是不是也和她一样，心里的激动和喜悦几乎无法抑制。"
#音色一:男-铿锵有力
...
#manual_seed = 507 #温柔100% 女声
#manual_seed = 1000 #播音 男声
PROMPT = "[speed_7]"
TOTAL_FILES = 10000
...
manual_seed_array = list(range(1, 10001))
num_folders = (len(manual_seed_array) // FILES_PER_FOLDER) + 1
for folder_index in range(num_folders):
    folder_name = f"result/{folder_index:04d}"
    speaker_folder = os.path.join(folder_name, "speaker")
    os.makedirs(folder_name, exist_ok=True)
    os.makedirs(speaker_folder, exist_ok=True)
    start_idx = folder_index * FILES_PER_FOLDER
    end_idx = min((folder_index + 1) * FILES_PER_FOLDER, len(manual_seed_array))

    for manual_seed in manual_seed_array[start_idx:end_idx]:
        with TorchSeedContext(manual_seed):
            rand_spk = chat.sample_random_speaker()
        file_name = f"{manual_seed:05d}"
        wav_file_path = os.path.join(folder_name, f"{file_name}.wav")
        pt_file_path = os.path.join(speaker_folder, f"{file_name}.pt")

        os.makedirs(folder_name, exist_ok=True)
        os.makedirs(speaker_folder, exist_ok=True)

        torch.save(rand_spk, pt_file_path)
        new_text = chat.infer(
            text,
        ...
        wavs = chat.infer(
            replace_arabic_with_chinese(new_text),
            skip_refine_text=True,
            params_infer_code=params_infer_code,
        )

        for i, wav in enumerate(wavs):
            wavfile.write(wav_file_path, 24000, np.array(wav))
        # t = np.linspace(0, 1, 24000)
        # frequency = 440  # 频率为440Hz（A4音符）
        # sine_wave = (np.sin(2 * np.pi * frequency * t) * 32767).astype(np.int16)  # 正弦波数据
        # wavfile.write(wav_file_path, 24000, sine_wave)
